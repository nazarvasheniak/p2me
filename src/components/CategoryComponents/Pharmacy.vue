<template>
  <div>
    <div class="">
      ...
      <div class="mainCatSelectContainer flex borderBottom">
        <div class="mainCatSelectItem" v-for="type in pharmacyCategory" :key="type.id">
          <label>{{ type.name }}
            <input type="checkbox"
                   name="subCatSelect"
                   v-model="shopType"
        ...
        <button @click="uploadType">ok</button>
      </div>
    </div>
    <div>
      <div v-for="type in pharmacyCategory" v-if="showSection">
...
<script>
  ...
  export default {
    ...
    computed: {
      pharmacyCategory() {
        const pharmacyCategory = this.$store.getters.getPharmacyCategory;
        localStorage.setItem('pharmacy_category', JSON.stringify(pharmacyCategory));
