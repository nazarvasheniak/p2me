<template>
  <div class="">
    ...
    <div class="mainCatSelectContainer borderBottom flex">
      <div class="mainCatSelectItem" v-for="type in restaurantCategory">
        <label>{{ type.name }}
          <input type="checkbox"
                 name="subCatSelect"
                 v-model="shopType"
...
<script>
  ...
  export default {
    ...
    computed: {
      restaurantCategory() {
        const restaurantCategory = this.$store.getters.getRestaurantCategory;
        localStorage.setItem('restaurant_category', JSON.stringify(restaurantCategory));
